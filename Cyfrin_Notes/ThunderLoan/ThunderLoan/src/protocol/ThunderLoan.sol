...
contract ThunderLoan is Initializable, OwnableUpgradeable, UUPSUpgradeable, OracleUpgradeable {
    ...
    function deposit(IERC20 token, uint256 amount) external revertIfZero(amount) revertIfNotAllowedToken(token) {
        AssetToken assetToken = s_tokenToAssetToken[token]; // token creation
        ...
        assetToken.updateExchangeRate(calculatedFee); // question: why we need to update the exachange rate ? => solved
            // this is a bug

        token.safeTransferFrom(msg.sender, address(assetToken), amount);
        // @audit  not safeTransfer(3 params) it should be  safeTransferFrom(4 params) funtion !
        // answer:  it is using internally using SafeERC20 for IERC20; so automatically it will be inserted into the
        // function.
    }
        ...
        revertIfNotAllowedToken(token)
    {
        ...
        uint256 exchangeRate = assetToken.getExchangeRate();
        // question why the heck this if contiton ? , becauses it gives a signal [type(uint256).max ]that redeem all the
        // balance
    ...
    // question only user can call this one , Not LP
    // answered: After a flash loan finishes, the protocol’s state must be exactly as if the loan never happened
    // (except for fees).
    // If this invariant holds, who calls the flash loan does not matter.
    function flashloan(
        address receiverAddress, // user's address
        IERC20 token,
        uint256 amount,
        ...
        revertIfNotAllowedToken(token)
    {
        ...
        s_currentlyFlashLoaning[token] = false; // question no need here , only need when flashLoan user call repay,
            // answered it is a bug
    }

    // @audit no natspec information about this function

    function repay(IERC20 token, uint256 amount) public {
        //question no amount limit check and also whethere amount has incluceded fee or not ?
        // answered it is design flaw , ⚠️ assumed implicitly
    ...
    function setAllowedToken(IERC20 token, bool allowed) external onlyOwner returns (AssetToken) {
        ...
        // @question why else conditon , here we are only setting the token as Allowed and not NotAllowed
        // answered: we are doing both but function name is misnamed
        else {
            AssetToken assetToken = s_tokenToAssetToken[token];
    ...
    function updateFlashLoanFee(uint256 newFee) external onlyOwner {
        // s_flashLoanFee is stored as a fraction scaled by s_feePrecision (e.g. 1e18).
        // newFee is NOT a percentage, but a ratio where:
        // newFee / s_feePrecision = actual fee rate.
        // Therefore, checking newFee <= s_feePrecision ensures the fee is at most 100%.
        // Comparing against the old s_flashLoanFee would incorrectly restrict governance
        // updates and does not validate the fee's correctness.
